...
<div class="container">
    ...
    <form method="post" action="/projetos-view/${projeto.id}/delete" style="margin-bottom:20px;">
        <button class="deletar" type="submit">Deletar Projeto</button>
    </form>
    ...
    <ul style="list-style: none; padding: 0;">
        ...
        <li style="margin-bottom: 10px; display: flex; align-items: center;">
            <input type="checkbox" disabled <#if t.status == "Concluida">checked</#if> style="margin-right: 10px;">
            <#if t.status == "Concluida"><s>${t.titulo}</s><#else>${t.titulo}</#if>
            <div style="margin-left: auto;" class="acoes">
                <form style="display:inline" method="post" action="/projetos-view/${projeto.id}/tarefas/${t.id}/toggle">
                    <button type="submit" style="margin-left: 10px;">
                        <#if t.status == "Concluida">Reabrir<#else>Concluir</#if>
                    </button>
                </form>
                <a href="/projetos-view/${projeto.id}/tarefas/${t.id}/editar" style="margin-left: 10px;"><button type="button" class="editar">Editar</button></a>
                <form style="display:inline" method="post" action="/projetos-view/${projeto.id}/tarefas/${t.id}/delete" style="margin-left: 10px;">
                    <button type="submit" class="deletar">Excluir</button>
                </form>
            </div>
        </li>
        </#list>
